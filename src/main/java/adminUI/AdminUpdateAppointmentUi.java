...
public class AdminUpdateAppointmentUi {
    public AdminUpdateAppointmentUi(Appointment appointment, Patient patient){
        ...
        JLabel title=new JLabel("UPDATE APPOINTMENT");
...
//        JLabel previouscontact=new JLabel(appointment.getContact());
        ...
        dateBox.setText(appointment.getDate());
        JLabel time=new JLabel("Time");
        JTextField timeBox=new JTextField(18);
        timeBox.setText(appointment.getTime());
...
//        patientContactBox.setText(appointment.getContact());
        ...
        updateAppointment.setBounds(530,600,200,40);
        ...
        updateAppointment.addActionListener(btn->{
...
//            JOptionPane.showMessageDialog(frame,"You need to pay bill");
            if(dateBox.getText().replaceAll("\\s","").equals("")||timeBox.getText().replaceAll("\\s","").equals("") ||patientNameBox.getText().replaceAll("\\s","").equals("") || patientAddressBox.getText().replaceAll("\\s","").equals("") || patientContactBox.getText().replaceAll("\\s","").equals("")||patientAgeBox.getText().replaceAll("\\s","").equals("") || patientGenderBox.getText().replaceAll("\\s","").equals("")){
                ...
                int doctor_id = DoctorService.getDoctorIdByName(doctorsNameBox.getSelectedItem().toString());
                int patient_id = PatientService.getPatientIdByContact(patient.getPatientContact());
                boolean flag = AppointmentService.updateAppointment(appointmentNumberBox.getText(),dateBox.getText(),timeBox.getText(),doctor_id,patient_id,patient.getPatientContact());
                boolean flag1 = PatientService.UpdatePatient(patientNameBox.getText(),patientAddressBox.getText(),patientContactBox.getText(),patientAgeBox.getText(),patientGenderBox.getText(),appointment.getContact());
                if(flag&&flag1){
                    JOptionPane.showMessageDialog(frame,"Appointment is updated");
                }
                else {
                    JOptionPane.showMessageDialog(frame, "Appointment is not updated");
                }
            }
...
//            }
        ...
        bottomPanel.add(updateAppointment);
